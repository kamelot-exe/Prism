...
impl GmailClient {
    ...
    pub async fn get_authorization_url(&self) -> Result<(String, String)> {
        self.oauth_client.get_authorization_url().await
    }

    pub async fn exchange_code(&self, code: String, state: String) -> Result<TokenInfo> {
        self.oauth_client.exchange_code(code, state).await
    }
